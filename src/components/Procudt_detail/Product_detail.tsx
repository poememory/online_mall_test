...
import { Button, Divider, Drawer, message, Rate } from "antd";
...
export default function Product_detail(){
    ...
    useEffect(()=>{
        ...
        const sendNumbers = async () => {
            ...
            if(token){
              const cartDetail=await fetchApi({
                url: '/cart',
                method: "GET",
              });           
            setInCart(cartDetail.cart.includes(P_ID))
            }
          };
          sendNumbers()
    },[P_ID])
    useEffect(() => {
      const chartDom = document.getElementById(`product_connection`) as HTMLElement;
        ...
        series: [
          {
            ...
            links: [
              {
                source:  'Now',
                target:  content[kind][0][1]
              },{
                source:  'Now',
                target:  content[kind][1][1]
              },{
                source:  'Now',
                target: content[kind][2][1]
              },{
                source:  'Now',
                target:  content[kind][3][1]
              },{
                source:  'Now',
                target:  content[kind][4][1]
              }
            ],
            lineStyle: {
              opacity: 0.9,
      ...
      myChart.on('click', function (params) {
        if(params.data.itemStyle.customData.id)nav(`/product_detail/${params.data.itemStyle.customData.id}`)       
    });
      // 在组件卸载时销毁图表，避免内存泄漏
      return () => {
        myChart.dispose();
        ...
        <div className="Product_detailPage">
          <Drawer title="选购渠道" size='large' onClose={onClose} open={open}>
            ...
            <div className="Product_detail">
                ...
                <div id={`product_connection`} style={{width:'40vw',height:'35vw',position:'absolute',top:'90vh',left:'10vw'}}></div>
            </div>
        </div>
    )
}